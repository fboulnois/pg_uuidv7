...
static Datum uuid_v7_to_timestamp_(pg_uuid_t *uuid, bool with_tz)
{
	uint64_t ts;
...
{
	return uuid_v7_to_timestamp_(PG_GETARG_UUID_P(0), true);
}

Datum uuid_v7_to_timestamp(PG_FUNCTION_ARGS)
{
	return uuid_v7_to_timestamp_(PG_GETARG_UUID_P(0), false);
}

static Datum uuid_timestamp_to_v7_(uint64_t ts, bool zero)
{
	pg_uuid_t *uuid = palloc(UUID_LEN);
	uint64_t tms;
...
Datum uuid_timestamptz_to_v7(PG_FUNCTION_ARGS) {
	...
	return uuid_timestamp_to_v7_((uint64_t)ts, zero);
}
